...
if [[ "$HOUR_NOW" == "00" || "$HOUR_NOW" == "12" ]]; then
  ...
  echo "./scripts/scheduler/create_job.sh ./cases/aututune_xla_meta.csv \"\" $TAG AUTOTUNE_XLA_META DEFAULT \"PROFILE=0\""
  ./scripts/scheduler/create_job.sh./cases/aututune_xla_meta.csv "" $TAG AUTOTUNE_XLA_META DEFAULT "PROFILE=0"
